...
search_decision_prompt = PromptTemplate(
    ...
    template=(
        "YOUR JOB: Decide if the user's question needs a web search to answer.\n\n"
        "OUTPUT FORMAT: Return exactly one word: SEARCH or NO_SEARCH\n\n"
        "The current date is the true current date.\n"
        "CURRENT DATE: {current_year}-{current_month}-{current_day}\n\n"
        "User question:\n{user_question}\n\n"
        "Conversation context:\n{conversation_history}\n\n"
        "Known answers:\n{known_answers}\n\n"
        "DECISION LOGIC:\n"
        "1. Is this a greeting, thanks, or simple acknowledgment? → NO_SEARCH\n"
        "2. Is ALL needed information already in Known answers or Conversation history above and you're confident you can answer without extra info? → NO_SEARCH\n"
        "3. Is it pure reasoning: math, coding, logic, text manipulation? → NO_SEARCH\n"
        "4. Is it general knowledge you are COMPLETELY CONFIDENT you know? → NO_SEARCH\n"
        "5. Does it need external facts, current data, or specific information? → SEARCH\n"
        "6. Are you NOT COMPLETELY CONFIDENT you can fully answer without extra info? → SEARCH\n"
        "7. When uncertain → SEARCH\n\n"
        "IMPORTANT: Only choose NO_SEARCH when COMPLETELY CONFIDENT you can fully answer without external information.\n\n"
        "EXAMPLES - NO_SEARCH:\n"
        "Question: 'Hi' → NO_SEARCH (greeting)\n"
        "Question: 'Thanks!' → NO_SEARCH (acknowledgment)\n"
        "Question: 'Implement a sorting algorithm' → NO_SEARCH (pure coding)\n"
        "Question: 'Solve 3x + 7 = 22' → NO_SEARCH (pure math)\n"
        "Question: 'What is a biological process in plants?' → NO_SEARCH (general knowledge)\n"
        "Question: 'Explain how a programming concept works' → NO_SEARCH (general concept)\n"
        "Question: 'Write a poem about nature' → NO_SEARCH (creative task)\n"
        "Known answers: 'A country capital is X.' / Question: 'What capital did you mention?' → NO_SEARCH (info in history)\n\n"
        "EXAMPLES - SEARCH:\n"
        "Question: 'What's the weather today?' → SEARCH (current data)\n"
        "Question: 'When was a famous landmark built?' → SEARCH (specific historical fact)\n"
        "Question: 'Who is the current leader of a country?' → SEARCH (current fact)\n"
        "Question: 'What's the stock price of a company?' → SEARCH (current data)\n"
        "Question: 'How do I install the latest version of a software tool?' → SEARCH (needs current version)\n"
        "Question: 'Write a comprehensive essay on recent developments in a research field' → SEARCH (needs current data)\n"
        "Context: Asked about a person / Question: 'When was he born?' → SEARCH (specific fact)\n\n"
        "TRICKY CASES:\n"
        "Question: 'Write an essay on a philosophical topic' → NO_SEARCH (general reasoning)\n"
        "Question: 'Write an essay on recent events' → SEARCH (needs current info)\n"
        "Question: 'What is a technical concept?' → NO_SEARCH (general knowledge)\n"
        "Question: 'What's the latest breakthrough in a field?' → SEARCH (needs current info)\n"
        "Question: 'Compare two approaches' → NO_SEARCH (general comparison)\n"
        "Question: 'Which product should I buy?' → SEARCH (needs current reviews)\n\n"
        "OUTPUT: SEARCH or NO_SEARCH"
    ),
)
