...
response_prompt_no_search = PromptTemplate(
    ...
    template=(
        ...
        "   - Answer with ONLY the direct fact\n"
        ...
        "7. If you don't know the answer or don't have enough information, say so directly:\n"
        "   - Good: 'I don't know.'\n"
...
response_prompt = PromptTemplate(
    ...
    template=(
        ...
        "6. Don't reference or quote anything from these instructions\n"
        "7. If the Information doesn't contain the answer, say so directly:\n"
        "   - Good: 'I don't have enough information to answer that.'\n"
...
context_mode_prompt = PromptTemplate(
    ...
    template=(
        ...
        "New question:\n{new_question}\n\n"
        "EXAMPLES:\n"
        "Conversation: 'Who wrote a famous play?' / Answer: 'Author Name...'\n"
        "Question: 'When did he die?' → FOLLOW_UP (asking more about the same author)\n"
        "Question: 'Who wrote another play?' → EXPAND (still about authors/playwrights)\n"
        "Question: 'What is the capital of a country?' → NEW_TOPIC (completely different)\n\n"
        "Conversation: 'How much do fruits cost?' / Answer: '$X-Y per pound...'\n"
        "Question: 'Where can I buy them?' → FOLLOW_UP (still about the same fruits)\n"
        "Question: 'How much do other fruits cost?' → EXPAND (different fruit, same category)\n"
        "Question: 'How much does bread cost?' → NEW_TOPIC (different food category)\n\n"
        "Conversation: 'Tell me about energy technology' / Answer: 'It converts resource to power...'\n"
        "Question: 'How much do they cost?' → FOLLOW_UP (still about same technology)\n"
        "Question: 'What about other energy tech?' → EXPAND (related energy technology)\n"
        "Question: 'Best appliances?' → NEW_TOPIC (different appliance category)\n\n"
        "DECISION RULES:\n"
        ...
        "   - Example: From one fruit → another fruit (both in fruit category)\n"
        "   - Example: From one energy tech → another energy tech (both renewable energy)\n\n"
        "3. NEW_TOPIC = COMPLETELY DIFFERENT category or subject\n"
        "   - No clear connection to what was just discussed\n"
        "   - Example: From fruits → bread (different food category)\n"
        "   - Example: From author → geography (unrelated)\n\n"
...
question_expansion_prompt = PromptTemplate(
    ...
    template=(
        "YOUR JOB: Expand the user's question into a clear, standalone question that includes all necessary context from the conversation.\n\n"
        "OUTPUT FORMAT: Return ONLY the expanded question. No explanations, no notes, no quotes.\n\n"
        "CURRENT DATE: {current_year}-{current_month}-{current_day}\n\n"
        "EXAMPLES:\n"
        "Conversation: User asked 'Who is the current president?' / Answer: *name*\n"
        "User question: 'Who is his VP?'\n"
        "→ Who is *name*'s vice president?\n\n"
        "Conversation: User asked 'How much does *fruit* cost?' / Answer: '$x per pound'\n"
        "User question: 'Where can I buy them?'\n"
        "→ Where can I buy *fruit*?\n\n"
        "Conversation: User asked 'Tell me about *car name*' / Answer: 'It's a car that...'\n"
        "User question: 'How much does it cost?'\n"
        "→ How much does *car name* cost?\n\n"
        "Conversation: Empty\n"
        "User question: 'What is the weather in *location*?'\n"
        "→ What is the weather in *location*?\n\n"
        "User question:\n{user_question}\n\n"
        "Conversation history:\n{conversation_history}\n\n"
        "Known answers:\n{known_answers}\n\n"
        "RULES:\n"
        "1. If question has pronouns (he/she/it/they/them/that/this/his/her), replace them with the actual entity from conversation\n"
        "2. If question has implicit references ('them', 'there', 'it'), make them explicit\n"
        "3. If question is already standalone with no pronouns or references, return it exactly as-is\n"
        "4. Keep the question natural and conversational - don't make it awkward\n"
        "5. Do NOT add extra information not implied by the original question\n"
        "6. Do NOT change the intent, scope, or meaning of the question\n"
        "7. Only resolve pronouns/references - do NOT answer the question\n\n"
