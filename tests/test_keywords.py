...
def test_extract_keywords_keeps_accented_english_tokens() -> None:
    ...
    keywords = K._extract_keywords(text)
...
def test_looks_like_followup_detects_short_pronoun_question() -> None:
    assert K._looks_like_followup("how much does it cost?", {"gpu"}) is True
    assert K._looks_like_followup("explain gpu memory bandwidth", {"gpu"}) is False
